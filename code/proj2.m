...
% (6) Evaluates the matches based on ground truth correspondences
 ...
 close all
CoreNum=4; % Change this parameter according to the number of your core in your computer
...
window = 50;
threshold = 0.002;
[x1, y1] = get_interest_points(image1_bw, threshold,window);
[x2, y2] = get_interest_points(image2_bw, threshold,window);

%C1 = corner(image1_bw,'Harris');
figure(100);
imshow(image1_bw);
hold on
plot(x1, y1, 'r*');
%C2 = corner(image2_bw,'Harris');
figure(101);
imshow(image2_bw);
hold on
plot(x2, y2, 'r*');
...
%[image1_features] = get_GLOH_features(image1_bw,x1,y1,24); 
%[image2_features] = get_GLOH_features(image2_bw,x2,y2,24);
